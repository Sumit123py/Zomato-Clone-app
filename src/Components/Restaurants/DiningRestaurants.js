...
const DiningRestaurants = () => {
    ...
    useEffect(() => {
      if(Ordervalue !== null){
        setSearchFilter(Ordervalue === 1 ? (RestaurantSearchData !== null ? (DiningRestaurantData.filter((item) => item.info.name.toLowerCase().includes(RestaurantSearchData))) : DiningRestaurantData ) : DiningRestaurantData)
      }
